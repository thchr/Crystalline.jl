...
"""
    corner_charge(brs::BandRepSet, Qd::Dict{String, Rational{Int}})

Compute a corner charge formula in the irrep multiplicities of a band representation set 
`brs`.
The corner charge assigned to a minimal set of bands occupying each individual Wyckoff
position must be provided as a dictionary `Qd`.

## Example
We may compute the corner charge formula in plane group 2:
```jl
julia> Qd = Dict("1a"=>0//1, "1b"=>0//1, "1c"=>0//1, "1d"=>1//2)
julia> corner_charge(bandreps(2, 2), Qd)
Q = (1/4)×(3Y₁ + 3B₁ + A₁ + Γ₁) (mod 1)
"""
function corner_charge(brs::BandRepSet, Qd::Dict{String, Rational{Int}})
    ...
    println("0: ", coefs)
    println("A: ", evenmod.(coefs, ambiguity))
    coefs .= evenmod.(coefs, ambiguity)
    println("B: ", evenmod.(coefs, ambiguity))
...
function pretty_compatibility_relations(brs)
    ...
    return [LinearRelation(irlabs, Vector(r)) for r in eachrow(C)]
end
