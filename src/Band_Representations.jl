...
function make_bandrep_set(sgnum::Integer; D::Integer=2)
    ...
    irdims=[([[size(lgg.matrices[1])[1] for lgg in lg] for (klab, lg) in get_lgirreps(sgnum, 2)]...)...]
    ...
    for wp in wps
        #isspecial(qvec(wp)) || continue 
        isspecial(wp) || continue 
        sitesym=_find_isomorphic_parent_pointgroup(SiteGroup( sg, wp)).label
...
function writebandreps(sgnum)
    filename=(@__DIR__)*"/../data/bandreps/2d/elementary/maxpaths/$(string(sgnum)).jld2"
    bandrepset_sgnum=make_bandrep_set(sgnum, D=2)
    @save filename bandrepset_sgnum 
end