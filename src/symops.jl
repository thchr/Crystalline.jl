...
Note that the generators are not in general _minimal_ in the sense that it may be possible
to generate the same group with fewer generators (an example is space group 168 (P6), which
could be generated by 6₀₀₁⁺ but returns [2₀₀₁, 3₀₀₁⁺]). The motivation for this is to expose
as similar generators as possible for similar crystal systems (see e.g. Section 8.3.5 of
the International Tables of Crystallography, Vol. A, Ed. 5 (ITA) for further background).

Additionally, we do not include the identity operation among the generators (contrary to the
conventions of ITA), except in space group 1, since they compose trivially.
"""
