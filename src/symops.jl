...
""" 
    get_sgops(sgnum::Integer, dim::Integer=3; verbose::Bool=false) --> SpaceGroup
...
not necessarily primitive. If desired, operations on a primitive unit
cell can be generated by multiplying with an appropriate transformation
matrix.
...
"""
function get_sgops(sgnum::Integer, dim::Integer=3; verbose::Bool=false)
...
nonsymmorphic (false).
"""
issymmorph(sgnum::Integer, dim::Integer=3) = issymmorph(get_sgops(sgnum, dim; verbose=false))
...
multtable(sg::AbstractGroup) = multtable(operations(sg))
