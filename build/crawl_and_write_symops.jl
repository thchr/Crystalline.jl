...
for i = 1:230
    Crystalline_str = Crystalline.crawl_Crystalline_xyzt(i)
    filename = (@__DIR__)*"/../data/Crystalline/3d/"*string(i)*".json"
    open(filename; write=true, create=true, truncate=true) do io
        JSON2.write(io, Crystalline_str)
    end 
end

# ----- NOW-REDUNANT FUNCTIONS FOR CRAWLING 3D SPACE GROUPS FROM BILBAO -----
""" 
    crawl_Crystalline_xyzt(sgnum::Integer, D::Integer=3)
...
"""
function crawl_Crystalline_xyzt(sgnum::Integer, D::Integer=3)
    htmlraw = crawl_Crystalline_html(sgnum, D)

    ops_html = children.(children(last(children(htmlraw.root)))[4:2:end])
    Nops = length(ops_html)
    Crystalline_str = Vector{String}(undef,Nops)

    for (i,op_html) in enumerate(ops_html)
        Crystalline_str[i] = _stripnum(op_html[1].text) # strip away the space group number
    end
    return Crystalline_str
end

function crawl_Crystalline_html(sgnum::Integer, D::Integer=3)
