...
@testset "SU(2) entries" begin
    ...
    @test all(1:MAX_SGNUM[3]) do sgnum
        all(spacegroup(sgnum, Val(3), Val(true))) do dop
...
@testset "SU2" begin
    u = su2(pointgroup("m-3m", Val(3), Val(true))[5])
    @test plain(u) == """
        SU(2) element:
         (1-i)/2  -(1+i)/2
         (1-i)/2   (1+i)/2"""
...
@testset "DSymOperation" begin
    g = pointgroup("m-3m", Val(3), Val(true))
    @test plain(g[2]) == """
        2₀₀₁ ─────────────────────────── (-x,-y,z)
         ┌ -1  0  0 ╷ 0 ┐
         │  0 -1  0 ┆ 0 │  ┌ -i  0 ┐
    ...
    h = pointgroup("6/mmm", Val(3), Val(true))
    @test plain(h[7]) == """
        2₁₁₀ ──────────────────────────── (y,x,-z)
         ┌ 0  1  0 ╷ 0 ┐
         │ 1  0  0 ┆ 0 │  ┌         0  -1/2-i√3/2 ┐
         └ 0  0 -1 ╵ 0 ┘, └ 1/2-i√3/2           0 ┘"""
    @test plain(spacegroup(230, Val(3), Val(true))[30]) == """
        {-3₁₋₁₁⁻|½,½,0} ───────── (-z+1/2,x+1/2,y)
         ┌ 0  0 -1 ╷ 1/2 ┐
