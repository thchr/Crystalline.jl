...
@testset "Herring criterion" begin
    ...
    for sgnum = 1:230
        Crystalline = operations(LGIRS[sgnum][1][1]) # this is important: we may _not_ use trivial repeated sets, i.e. spacegroup(..) would not work generally
        for kidx = 1:length(LGIRS[sgnum])
            for iridx = 1:length(LGIRS[sgnum][kidx])
                ...
                #= try =#       # for debugging
                herring_type = herring(LGIRS[sgnum][kidx][iridx], Crystalline)
